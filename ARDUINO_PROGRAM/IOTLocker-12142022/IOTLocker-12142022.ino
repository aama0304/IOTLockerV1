...
LiquidCrystal_I2C  lcd2(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); //LCD Module for Locker 2 [CHANGE I2C ADDRESS]
LiquidCrystal_I2C  lcd3(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); //LCD Module for Locker 3 [CHANGE I2C ADDRESS]
LiquidCrystal_I2C  lcd4(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); //LCD Module for Locker 4 [CHANGE I2C ADDRESS]
...
void setup()
{
  ...
  RfidMod2.PCD_DumpVersionToSerial();
  RfidMod3.PCD_DumpVersionToSerial();
  RfidMod4.PCD_DumpVersionToSerial();
...
void ReadRFID2()
{
  if ( ! RfidMod2.PICC_IsNewCardPresent())
  {
    return;
  }
  if ( ! RfidMod2.PICC_ReadCardSerial())
  {
    return;
  }

  byte letter;
  for (byte i = 0; i < RfidMod2.uid.size; i++)
  {
    //lcd.setCursor(0, 0);
    //Serial.print(RfidMod1.uid.uidByte[i] < 0x10 ? " 0" : " ");
    //Serial.print(RfidMod1.uid.uidByte[i], HEX);
    IDcontent2.concat(String(RfidMod2.uid.uidByte[i] < 0x10 ? " 0" : " "));
    IDcontent2.concat(String(RfidMod2.uid.uidByte[i], HEX));
  }

  IDcontent2.toUpperCase();
  if (IDcontent2.substring(1) == RFID2) // Make sure you change this with your own UID number
  {
    if (DisinfectLock2 == false)
    {
...
void ReadRFID3()
{
  if ( ! RfidMod3.PICC_IsNewCardPresent())
  {
    return;
  }
  if ( ! RfidMod3.PICC_ReadCardSerial())
  {
    return;
  }

  byte letter;
  for (byte i = 0; i < RfidMod3.uid.size; i++)
  {
    //lcd.setCursor(0, 0);
    //Serial.print(RfidMod1.uid.uidByte[i] < 0x10 ? " 0" : " ");
    //Serial.print(RfidMod1.uid.uidByte[i], HEX);
    IDcontent3.concat(String(RfidMod3.uid.uidByte[i] < 0x10 ? " 0" : " "));
    IDcontent3.concat(String(RfidMod3.uid.uidByte[i], HEX));
  }

  IDcontent3.toUpperCase();
  if (IDcontent3.substring(1) == RFID3) // Make sure you change this with your own UID number
  {
    if (DisinfectLock3 == false)
    {
...
void ReadRFID4()
{
  if ( ! RfidMod4.PICC_IsNewCardPresent())
  {
    return;
  }
  if ( ! RfidMod4.PICC_ReadCardSerial())
  {
    return;
  }

  byte letter;
  for (byte i = 0; i < RfidMod4.uid.size; i++)
  {
    //lcd.setCursor(0, 0);
    //Serial.print(RfidMod1.uid.uidByte[i] < 0x10 ? " 0" : " ");
    //Serial.print(RfidMod1.uid.uidByte[i], HEX);
    IDcontent4.concat(String(RfidMod4.uid.uidByte[i] < 0x10 ? " 0" : " "));
    IDcontent4.concat(String(RfidMod4.uid.uidByte[i], HEX));
  }

  IDcontent4.toUpperCase();
  if (IDcontent4.substring(1) == RFID3) // Make sure you change this with your own UID number
  {
    if (DisinfectLock4 == false)
    {
